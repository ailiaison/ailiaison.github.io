(function () {
  ...
  style.textContent =
    ...
    "}" +
    ".sd-root.sd-collapsed .sd-layout{" +
      "max-height:0;" +
      "opacity:0;" +
    "}" +
  ...
  // изначально свёрнут
  root.className = "sd-root sd-collapsed";
  root.innerHTML =
    '<div class="sd-header">' +
      '<div>' +
  ...
  // ---------- 3. HEADER TOGGLE (ACCORDION) ----------
  var headerEl = root.querySelector(".sd-header");
  if (headerEl) {
    headerEl.style.cursor = "pointer";
    var sdCollapsed = true; // соответствует начальному классу sd-collapsed

    headerEl.addEventListener("click", function () {
      sdCollapsed = !sdCollapsed;
      if (sdCollapsed) {
        root.classList.add("sd-collapsed");
      } else {
        root.classList.remove("sd-collapsed");
      }
    });
  }
  ...
  function syncDealFromForm() {
    ...
    deal.commodity = fCommodity ? (fCommodity.value || "").trim() : "";
    deal.exporter = fExporter ? (fExporter.value || "").trim() : "";
    deal.currency = fCurrency ? (fCurrency.value || "").trim() : "";
    deal.market = fMarket ? (fMarket.value || "").trim() : "";
