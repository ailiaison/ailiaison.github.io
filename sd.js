...
(function () {
    ...
    '<div class="sd-layout">' +
      '<div class="sd-col sd-col-left">' +
        '<div class="sd-card">' +
          ...
          "<p>Describe your trade and launch the simulated signal engine.</p>" +
          '<form id="sdDealForm">' +
  ...
  // ---------- 5. CHART INIT ----------
  if (hasChartJs) {
    const indexCtx = root.querySelector("#sdIndexChart")?.getContext("2d") || null;
    ...
    pairs.forEach(function (p, idx) {
      const canvas = root.querySelector("#" + p.chartId);
      if (!canvas) return;
      const c = canvas.getContext("2d");
      // чуть разные оттенки
      const colors = ["#38bdf8", "#22c55e", "#f97373", "#facc15"];
      const fills = [
        "rgba(56,189,248,0.2)",
        "rgba(34,197,94,0.2)",
      ...
      pairCharts[p.key] = makeLineChart(c, p.key, colors[idx % colors.length], fills[idx % fills.length]);
    });
  }
  ...
  function step() {
    ...
    pairs.forEach(function (p) {
      ...
      const history = pairHistory[p.key];
      history.push(rate);
    ...
    const trendInfo = classifyTrendText(diff);
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      updateSummary();
      step(); // сразу «подвинуть» симуляцию под ввод сделки
    });
  }
